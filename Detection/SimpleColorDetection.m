...
try
	...
	if strcmpi(reply2, 'My Own') || strcmpi(selectedImage, 'Canoe') > 0
		...
		if eightBit
			redThresholdHigh = uint8(redThresholdHigh * 255);
			greenThresholdLow = uint8(greenThresholdLow * 255);
		...
		greenThresholdLow = 70;
		greenThresholdHigh = 150;
		blueThresholdLow = 0;
	...
	subplot(3, 4, 11);
	imshow(redMask, []);
	title('Is-Not-Red Mask', 'FontSize', fontSize);
	subplot(3, 4, 12);
	imshow(blueMask, []);
	...
	greenObjectsMask = cast(greenObjectsMask, class(greenBand));
	
	% Use the red object mask to mask out the red-only portions of the rgb image.
	maskedImageR = greenObjectsMask .* redBand;
	maskedImageG = greenObjectsMask .* greenBand;
	maskedImageB = greenObjectsMask .* blueBand;
...
try
	...
	if ~strcmpi(class(greenBand), 'uint8')
